...
for j in range(eigenfunc_num):
	...
	colouringdict['eigfunc_'+str(j+1)] = colouring.copy()
...
if write_ply:    
	write_custom_colour_ply_file(tm=surf_tm, colouringdict = colouringdict, filepath='../data/visualisation/eigenfunc/'+surf_name+'/'+surf_name+'.ply') 
...
if show_cotan:
	...
	sparse_L_cotan_matrix = Minv.sqrt() @ C @ Minv.sqrt()
	L_cotan_matrix = sparse_L_cotan_matrix.todense().T
	
	vals, vecs = sparse.linalg.eigs(sparse_L_cotan_matrix, k=num_eigenfuncs, which='SM')
	
	
	for i in range(num_eigenfuncs):
		print('discrete eigenval',i, 'is',vals[i])
		
		
		if plotting:
			plt.plot(vecs[:,i] ) #/(    (((vecs[:,i])**2).sum())**(0.5) )    )
			plt.show()
		
		print(vecs[:,i])
		cotan_colouring = cmap(vecs[:,i] * discrete_function_scaling + 0.5 )
		
		surf_tm.visual.vertex_colors=cotan_colouring
		surf_tm.vertices = surf_tm.vertices + np.array([4,0,0])
		
		mesh_rd2 = pyrender.Mesh.from_trimesh(surf_tm)
		
		show_mesh_gui([mesh_rd1, mesh_rd2])
