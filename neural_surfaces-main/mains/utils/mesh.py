...
def sphere_upsample_mesh(V, F, uv, threshold=0.2):
    import pymeshlab
	
    #return V, F, uv
    #This is just a placeholder.
    
    write_mesh('/tmp/file.obj', V, F, None, None)
    ms = pymeshlab.MeshSet()
    ms.load_new_mesh('/tmp/file.obj')
    
    write_mesh('/tmp/file.obj', uv, F, None, None)
    ms2 = pymeshlab.MeshSet()
    ms2.load_new_mesh('/tmp/file.obj')

    #p = pymeshlab.Percentage(0.0)
    p = pymeshlab.Percentage(0.0)
    ms.subdivision_surfaces_midpoint(threshold=p)#force uniform refinement (to avoid mismatch between sphere and mesh vertices)
    ms.re_compute_vertex_normals()
    ms.re_compute_face_normals()
    ...
    ms2.subdivision_surfaces_midpoint(threshold=p)#force uniform refinement (to avoid mismatch between sphere and mesh vertices)
    ms2.re_compute_vertex_normals()
    ms2.re_compute_face_normals()

    mesh = ms2.current_mesh()

    UV_large  = mesh.vertex_matrix()
