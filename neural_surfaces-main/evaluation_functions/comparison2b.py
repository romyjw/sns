...
class Anal_Surface_Evaluator:
	...
	def SNS_curvatures(self, batch_size=2000):
		...
		weights = torch.load('../data/SNS/'+self.mesh_name+'/weights.pth', map_location=torch.device('cpu'))
	...
	def show_distributions(self, show_analytic=True):
		
				
		if show_analytic==True:
			draw_histogram_curve(self.H, range=[-50,20], bins=30, n=5000, label='analytic H', style='k-')	
		...
		draw_histogram_curve(self.SNS_H, range=[-50,20], bins=30, n=5000, label='SNS H '+self.mesh_name, style='-.')
		plt.legend()
	...
	def show_errors(self):
		...
		draw_histogram_curve(SNS_error, range=[0,1], bins=50, n=500, label='SNS H error '+self.mesh_name, style='-.')
		
		plt.legend()

		

name_mesh_dict = {'SPIKE' : ['S-analytic-SNS', 'S-remesh3', 'S-remesh4', 'S-remesh5', 'S-remesh6'],
					'LAUNDRY' : ['L-analytic-SNS', 'L-remesh3', 'L-remesh4', 'L-remesh5', 'L-remesh6'],
					'FLOWER' : ['F-analytic-SNS', 'F-remesh3', 'F-remesh4', 'F-remesh5', 'F-remesh6'],
					'TREE' : ['T-analytic-SNS', 'T-remesh3', 'T-remesh4', 'T-remesh5', 'T-remesh6']
					}
...
for mesh_name in name_mesh_dict[surf_name]:
